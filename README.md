...
| **Gemini** (Google) | • ตอบเร็ว<br>• ใช้งานได้กับคอมพิวเตอร์สเปคต่ำ<br>• มีโมเดลให้เลือกหลายระดับราคา | • ต้องต่ออินเทอร์เน็ต<br>• ข้อมูลถูกส่งออกนอกเครื่อง ไม่เหมาะกับเอกสารที่มีความลับ<br>• ต้องสมัครและจัดการ API Key<br>• มีค่าใช้จ่ายตามจำนวน Token ที่ใช้ |
...
- `_check_chunk_openai()` — ส่งคำถามไปยัง OpenAI API พร้อม exponential backoff retry สูงสุด `MAX_RETRIES` ครั้งสำหรับ HTTP 429 รองรับ o1/o3 series ด้วย
- `_check_chunk()` — router เลือก provider ตามชื่อโมเดล (`gemini*` → Gemini, `gpt*/o1*/o3*` → OpenAI, อื่นๆ → Ollama)
